...
class Manual_Player:
	...
	def move(self, board, old_move, flag):
		...
		for c in cells:
			...
			if flag=="x":
				d=minimax(0, maximise, float("-inf"), float("inf"), c, "o", board)
				if d>bestval:
					bestval=d
					selected=c
			if flag=="o":
				d=minimax(0, maximise, float("-inf"), float("inf"), c, "x", board)
				if d<bestval:
					bestval=d
					selected=c	
			board.big_boards_status[c[0]][c[1]][c[2]]=temp_big_boards_status
	...
	def heuristic(self,ply):
		...
		for k in range(2):
			for i in range(3):
				sumx = 0
				for j in range(3):
					sumx  = sumx +  hashx[k][i][j]/800
			    if sumx >= 0 and sumx <= 1 :
				     ohash = ohash + sumx
				elif sumx >=1 and sumx <= 2:
